...
LeoBuild.prototype.buildMain = function(project,dir,callback){
  ...
  if (this.verbose > 1) {
    out.log('  [compiler corePath]  ' + this.env.compiler.corePath);
    out.log('  [runtime build extentions]  ' + this.env.runtime.buildExtentions);
  }
  ...
  if (this.verbose > 1) {
    out.log('  [projectFiles] ' + projectFiles);
    out.log('  [mainFile] ' + mainFile);
  }
